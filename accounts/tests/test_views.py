...
class SendLoginEmailViewTest(TestCase):
    ...
    def test_redirects_to_home_page(self):
        response = self.client.post('/accounts/send_login_email', data={'email': 'sana@ya.ru'})
        self.assertRedirects(response, '/')

    @patch('accounts.views.send_mail')
    def test_send_mail_to_address_from_post(self, mock_send_mail):
        self.client.post('/accounts/send_login_email', data={'email': 'sana@ya.ru'})

        self.assertEqual(mock_send_mail.called, True)
        (subject, body, from_email, to_list), kwargs = mock_send_mail.call_args
        self.assertEqual(subject, 'Your login link for Superlists')
        self.assertEqual(from_email, 'sana256@yandex.ru')
        self.assertEqual(to_list, ['sana@ya.ru'])


    def test_adds_success_message(self):
        response = self.client.post('/accounts/send_login_email', data={'email': 'sana@ya.ru'}, follow=True)
    ...
    def test_creates_token_associated_with_email(self):
        ...
        self.assertEqual(token.email, 'sana@ya.ru')
