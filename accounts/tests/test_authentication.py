...
class GetUserTest(TestCase):
    ...
    def test_user_by_email(self):
        User.objects.create(email='another@ya.ru')
        desired_user = User.objects.create(email='sana@ya.ru')
        found_user = PasswordlessAuthenticationBackend().get_user('sana@ya.ru')
        self.assertEqual(found_user, desired_user)


    def test_return_None_if_no_user_with_that_email(self):
        self.assertIsNone(PasswordlessAuthenticationBackend().get_user('sana@ya.ru'))
